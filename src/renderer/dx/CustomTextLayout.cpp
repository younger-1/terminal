    ...
    _width{ width }
{
    ...
    _localeName.resize(static_cast<size_t>(format->GetLocaleNameLength()) + 1); // +1 for null
...
[[nodiscard]] HRESULT STDMETHODCALLTYPE CustomTextLayout::GetColumns(_Out_ UINT32* columns) noexcept
{
...
[[nodiscard]] HRESULT CustomTextLayout::_ShapeGlyphRun(const UINT32 runIndex, UINT32& glyphStart) noexcept
{
    try
    {
        ...
        const auto glyphBeginOffset = static_cast<size_t>(glyphStart) + static_cast<size_t>(actualGlyphCount);

        _glyphAdvances.resize(std::max(glyphBeginOffset, _glyphAdvances.size()));
        _glyphOffsets.resize(std::max(glyphBeginOffset, _glyphOffsets.size()));
                                                       ...
                                                       const D2D_POINT_2F origin) noexcept
{
    ...
    try
    {
        ...
        {
            // Get the run
            const auto& run = _runs.at(runIndex);
                                                                               ...
                                                                               UINT32 textLength)
{
    try
    {
        ...
        familyName.resize(static_cast<size_t>(format1->GetFontFamilyNameLength()) + 1);
